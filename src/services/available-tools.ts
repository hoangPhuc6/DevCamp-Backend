...
function getWeather(location: string): object {
  if (location.toLowerCase().includes('hanoi')) {
    return { location: 'Hanoi', temperature: '15°C', condition: 'Cloudy' };
  } else if (location.toLowerCase().includes('bangkok')) {
    return {
      location: 'Bangkok',
      temperature: '22°C',
      condition: 'Sunny',
...
export const functionDeclarations: FunctionDeclaration[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        location: {
          type: Type.STRING,
          description: 'The city, e.g. HCMC, Hanoi, Bangkok',
        },
      },
      required: ['location'],
    },
